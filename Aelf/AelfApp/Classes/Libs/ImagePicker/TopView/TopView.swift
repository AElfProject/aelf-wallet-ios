...
fileprivate let flashImageNames = ["Auto", "On", "Off"]
...
class TopView: UIView {
    ...
    let flashButtonTitles = flashImageNames.map({ $0.localized() })
    ...
    @objc func flashButtonDidPress(_ button: UIButton) {
        ...
        let imgName = flashImageNames[currentFlashIndex]
        let title = flashButtonTitles[currentFlashIndex]

        button.setImage(AssetManager.getImage(imgName), for: UIControl.State())
